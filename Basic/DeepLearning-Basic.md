...
딥러닝은 **신경망** 알고리즘을 사용하는 머신러닝의 한 분야로 뇌의 **신경세포의 구조**를 착안하여 고안된 알고리즘을 사용한다. 
...
우리 뇌는 비교적 단순한 정보를 처리하는 많은 신경세포들로 이루어졌다고 알려져 있다. 하나의 선경세포는 다른 신경세포로 부터 정보를 받아 새로운
정보를 생성하고 생성한 정보는 또 다른 신경세포로 전달된다. 하나의 신경세포를 인공적으로 모델릴한 뉴런인 **퍼셉트론(Perceptron)** 이 제안되었다.
...
**퍼셉트론**은 1957년 프랑크 로젠블라드(Frank Rosenblatt)에 의해 처음 고안된 오래된 알고리즘이다. 앞의 왼쪽 그림은 생물학적인 뉴런(neuron)을 나타내고 있다. 
오른쪽의 그림은 인공 뉴런이 퍼셉트론으로 다수의 입력을 받아 하나의 출력을 만든다. 이 퍼셉트론에서 가중치(weight)가 각각의 입력신호에 부여되어 신호의 총합이 정해진 임계값(\theta,세타)을 넘었을 때 1을 출력한다. 이는 뉴런의 활성화(activation)라고 표현한다. 활성화되면 1의 값을 가지고 활성화되지 않으면 0의 값을 가진다. 
퍼셉트론의 출력 값이 1 또는 0이기 때문에 선형 분류(linear classifier) 모형이라고 한다. 

퍼셉트론 알고리즘을 간단히 살펴보자.

뉴런하나로 입력되는 정보가 X1, X2 두 개이고 할 때 각 입력 값에 대한 가중치가  W1, W2 라고 하자. 
이 때 결과 값 Y는 X1 * W1 * X2 * W2 가 된다. 이 값이 어떤 기준과 비교하여 만족스러운 결과가 나올때 까지 W1, W2의 값을 조정해 나간다. 
그리고 그 결과 값은 다시 다른 뉴론으로 전달된다. 

퍼셉트론 하나가 처리하는 정보는 단순하나 수천 억에서 1조 개에 달하는 많은 수의 퍼셉트론이 서로 얽혀서 복잡한 정보를 가농할 수 있다. 

마빈 민스키(Marvin Minsky) 등이 단순한 퍼셉트론으로는 배타적 논리합(XOR) 문제를 풀 수 없다고 증명하였고 다층퍼셉트론으로는 XOR 연산이 가능함을 보였다. 

* 신경세포: 하나가 처리하는 정보는 단순하나 수턴 억에서 1조 개에 달하는 많은 수의 신경세포가 거미줄처럼 앍혀 있어 복잡한 정보를 가공할 수 있다. 
이들 신경 세포들은 여러 개의 계층을 이루어 역할을 담당한다.

* 신경먕: 여러 개의 뉴런이 하나의 계층을 하니의 계층을 형성하고 이런 계층을 다시 여러 개 쌓어 올린다. (기본적인 신경망은 입력층, 중간층, 출력층의 세 계층 구조 네트워크)

* 모델 학습 또는 훈련 과정: 준비된 표본 데이터를 입력받아 원하는 출력 데이터가 만들어지도록 연결된 뉴런끼리 얼마만큼 정보를 주고받을지 가중치를 매번 조정하여 신경망에 있는 모든 뉴런 간의 연결을 최적화하는 과정

* 추론 과정: 모델 학습이 완료된 후 샘플 데이터가 아닌 실제 상황의 데이터를 넣어 결과를 얻어내는 과정 
...
* 심층 신경망 (DNN: Deep Neural Network)
  ...
  * 중간 계층, 은닉 계층(Hidden Layer): 입력 계층과 출력 계층 사이의 하나 이상의 계층 
  
 * 신경망과 심층 신경망: 신경망 중 은닉 계층이 하나 이상 있는 신경망이 심층 신경망이다.  
  
![DNN(Deep Neural Network) 구조](../dnn.png)
...
구글에서 2015년 7월 딥드림(DeepDream) 이라는 CNN 구조 발표하여 서로 다른 이미지 패턴을 인식하고 합성하는 방식으로 몽환적인 이미지를 생성하였다.
...
* 딥 러닝의 석학들
  * 앤드루 응(Andrew Ng): 텐서플로우를 만든 구글 브레인팀을 이끌었고 바이두에 합류하고 딥러닝 프레임워크 [PaddlePaddle](http://www.paddlepaddle.org)을 오픈소스로 공개하였다.
  * 얀 르쿤(Yann LeCun): 뉴욕 대학교의 교수로 페이스북에 합류하였다. 
  * 제프이 힌튼(Geoffey Hinton): 토론토 대학교수로 구글에서 인공지능 분야를 이끌고 있다. 

## 주요 개발 내용 
* DenseCap - Stanford University
* DeepDream - Google
* DeepText - facebook
